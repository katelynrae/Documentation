...
You may be wondering whether, do you must use Server to run a Network Canvas study? Not necessarily. Network Canvas protocols can be designed in Architect and opened directly in Network Canvas. That is often good enough for very small work with one device. However, many researchers will be working with many cases, multiple devices, or have a need to keep all their case files in a single location. Server, thus, is often a useful companion to Network Canvas desgined for project workflows. 
...
Once imported, Server will create a workspace for this protocol file. It will be designated by protocol initials on the side bar. You can have multiple protocols / studies running on Server in parallel. Please note that once you have imported a protocol into Server it can be 'live' and this means that it is not possible to change most of the details about a protocol. This is by design. If a protocol is in the field and the quesion names and ordering are changed it will make it difficult if not impossible to export all this data in a single table.
...
Once you have imported a protocol, you can close the dialog and use the newly imported protocol to collect interview data. At the end of the interview, the final screen of the survey will have an "upload" button. Clicking that button will upload this particoular case to the Server. The upload will give feedback indicating whether the upload was successful. To be absolutely sure, you can check on Server since the overview panel will update the case IDs in real time. 
